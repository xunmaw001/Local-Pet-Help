        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '寻找宠物名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xuanshangName" placeholder="寻找宠物名称" clearable></el-input>
                     </el-form-item>
                                         
                     <el-form-item :label="contents.inputTitle == 1 ? '寻找宠物类型' : ''">
                        <el-select v-model="searchForm.xuanshangTypes" placeholder="请选择寻找宠物类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xuanshangTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xuanshang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chongwulingyangpingtai/upload/xuanshangMuBan.xls"
                        >批量导入寻找宠物数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xuanshang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xuanshangUploadSuccess"
                                :on-error="xuanshangUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xuanshang','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入寻找宠物数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xuanshang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xuanshang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="寻找宠物名称">
                        <template slot-scope="scope">
                            {{scope.row.xuanshangName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xuanshangUuidNumber"
                                   header-align="center"
                                   label="寻找宠物编号">
                        <template slot-scope="scope">
                            {{scope.row.xuanshangUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xuanshangPhoto"
                                     header-align="center"
                                     width="200"
                                     label="寻找宠物照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xuanshangPhoto">
                                <img :src="$base.url+scope.row.xuanshangPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="悬赏金额">
                        <template slot-scope="scope">
                            {{scope.row.xuanshangJine}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xuanshangAddress"
                                   header-align="center"
                                   label="寻找宠物地点">
                        <template slot-scope="scope">
                            {{scope.row.xuanshangAddress}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xuanshangTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xuanshang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xuanshang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xuanshang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              xuanshangTypesSelectSearch : [],
            form:{
                id : null,
                yonghuId : null,
                xuanshangName : null,
                xuanshangUuidNumber : null,
                xuanshangPhoto : null,
                xuanshangJine : null,
                xuanshangAddress : null,
                xuanshangTypes : null,
                xuanshangContent : null,
                xuanshangDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '寻找宠物名称': "xuanshangName",
                     '寻找宠物编号': "xuanshangUuidNumber",
                     '寻找宠物照片': "xuanshangPhoto",
                     '悬赏金额': "xuanshangJine",
                     '寻找宠物地点': "xuanshangAddress",
                     '寻找宠物类型': "xuanshangValue",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xuanshang',//当前表表名,
                        sumColum : 'xuanshang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xuanshang_name',//分组字符串字段
                        // types : 'xuanshang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.xuanshangName!= '' && this.searchForm.xuanshangName!= undefined) {
                    params['xuanshangName'] = '%' + this.searchForm.xuanshangName + '%'
                }
                                         
                if (this.searchForm.xuanshangTypes!= '' && this.searchForm.xuanshangTypes!= undefined) {
                    params['xuanshangTypes'] = this.searchForm.xuanshangTypes
                }
                                
                params['xuanshangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xuanshang_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xuanshangTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            xuanshangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xuanshang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
