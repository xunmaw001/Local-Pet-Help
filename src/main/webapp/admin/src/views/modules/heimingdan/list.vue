        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('heimingdan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chongwulingyangpingtai/upload/heimingdanMuBan.xls"
                        >批量导入黑名单数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('heimingdan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="heimingdanUploadSuccess"
                                :on-error="heimingdanUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('heimingdan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "heimingdan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="heimingdanUuidNumber"
                                   header-align="center"
                                   label="黑名单编号">
                        <template slot-scope="scope">
                            {{scope.row.heimingdanUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="heimingdanTypes"
                                      header-align="center"
                                      label="黑名单类型">
                        <template slot-scope="scope">
                            {{scope.row.heimingdanValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('heimingdan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('heimingdan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('heimingdan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                heimingdanUuidNumber : null,
                heimingdanTypes : null,
                heimingdanContent : null,
                heimingdanDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '黑名单编号': "heimingdanUuidNumber",
                     '黑名单类型': "heimingdanValue",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'heimingdan',//当前表表名,
                        sumColum : 'heimingdan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'heimingdan_name',//分组字符串字段
                        // types : 'heimingdan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['heimingdanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            heimingdanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "heimingdan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
